...
def creator_text():
    ...
    text = font.render('© 2018 Edgar Araújo ;)', True, color_black)
...
def show_highscore():
    ...
    text_surf, text_rect = text_objects('Highscore: ' + str(highscore), font, color_black)
...
def button1(mouse_pos, text, y, x=None, action=None, colour=color_green, font_size=20, width=None):
    ...
    if x + width > mouse_pos[0] > x and y + button_height > mouse_pos[1] > y:
        ...
        # Click action
        if action == 'play' or action == 'replay':
            game_loop()
        if action == 'continue':
            unpause()
...
def crash(score):
    ...
    while crashed:
        ...
        # Buttons
        button1(mouse_pos, 'REPLAY', 300)
        button2(mouse_pos, 'MAIN MENU', 380)
        button4(mouse_pos, 'QUIT', 460)
...
def pause(score):
    ...
    while paused:
        ...
        # Buttons
        button1(mouse_pos, 'CONTINUE', 300)
        button2(mouse_pos, 'MAIN MENU', 380)
        button4(mouse_pos, 'QUIT', 460)
...
def main_menu():
    ...
    while menu:
        ...
        button2(mouse_pos, 'CARS', 330)
        button3(mouse_pos, 'HELP', 390)
        button4(mouse_pos, 'QUIT', 450)
